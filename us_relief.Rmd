...
For the U.S. map, I'm going to use the county-level map contained in the `ussf` package. (More about that and related packages [here](https://dcl-docs.stanford.edu/)). This map includes the lower 48 states, and also a rescaled and relocated Hawaii and Alaska.
...
```{r first-cut, fig.fullwidth = TRUE}
plot(us_relief)
```
...
ggsave(file = "figures/base_out_inferno.png", 
       plot = base_out, width = 10, height=6, units="in")

```

```{r, fig.fullwidth = TRUE}
include_graphics("figures/base_out_inferno.png")
```
...
ggsave(file = "figures/base_and_county.png", plot = out, width = 10, height=6, units="in")

```

```{r, fig.fullwidth = TRUE}
include_graphics("figures/base_and_county.png")
```
